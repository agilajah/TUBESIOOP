...
class Makhluk{
	public:
		...
		void SetKekuatan(int kekuatan): kekuatan(kekuatan){};
		void SetPoint(int x, int y): Point(x,y){};
		int GetID() const {};
...
class Hewan : public Makhluk{ //Descendant from mahluk
	public:
		...
		//getter-setter
		//We don't have any setter, langkah can only able to set from member initializer (when we invoke this class)
